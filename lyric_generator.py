...
def generate_from_word_level_rnn(maxlen=30, diversity=1.0, min_sent_len=10, max_sent_len=65):
    with open("single_corpus.lyc", "r") as f:
    ...
    response = ""
    model = model_from_json(open('model.text').read())
    model.load_weights('model.weights')
    model.compile(loss='categorical_crossentropy', optimizer='rmsprop')
...
#    train()
print(generate_from_word_level_rnn(30))

# Separate quotes, commas, apostrophes, and newlines